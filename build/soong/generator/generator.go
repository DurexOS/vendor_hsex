...
func init() {
	android.RegisterModuleType("heavens_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = heavensExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = heavensExpandVariables(ctx, dep_file)
	...
	cmd := heavensExpandVariables(ctx, String(g.properties.Cmd))
